...
const { likedPosts } = storeToRefs(userProfileStore)

const loading = ref(false)
const loadErr = ref(false)
const currentPage = ref(0)
const totalPages = ref(0)
const likedPostsContainer = useTemplateRef('likedPostsContainer')
...
async function getLikedPosts() {
  ...
  try {
    let res = await api.get('/users/get-all-liked-posts', {
      params: {
    ...
    if (res.status === 200) {
      currentPage.value += 1
      likedPosts.value.push(...res.data['posts'])
...
function onUpdateLikedFlag(index, flag, updateLikeCount) {
  if (index !== -1) {
    likedPosts.value[index].user_liked = flag

    if (updateLikeCount) {
      if (flag) {
        likedPosts.value[index].like_count += 1
      } else {
        likedPosts.value[index].like_count -= 1
      }
    }
...
function onSetSaveBtnLoadingFlag(index, flag) {
  if (index !== -1) {
    likedPosts.value[index].saving = flag
  }
}

function onSetUnsaveBtnLoadingFlag(index, flag) {
  if (index !== -1) {
    likedPosts.value[index].unsaving = flag
  }
}

function onPostSavedSuccess(index) {
  if (index !== -1) {
    likedPosts.value[index].user_saved = true
  }
}

function onPostUnsavedSuccess(index) {
  if (index !== -1) {
    likedPosts.value[index].user_saved = false
  }
}
</script>
...
<template>
  ...
  >
    ...
    <div v-if="!loading && likedPosts.length === 0" style="text-align: center">
